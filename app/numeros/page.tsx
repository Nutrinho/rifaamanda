import { PublicShell } from "@/components/campaign-shell";
import { NumberGrid } from "@/components/number-grid";
import { getCampaign, getNumbers } from "@/lib/supabase";

export default async function NumerosPage() {
  const [campaign, numbers] = await Promise.all([getCampaign(), getNumbers()]);
  return (
    <PublicShell>
      <main className="mx-auto max-w-6xl px-4 py-8">
        <h1 className="text-3xl font-black text-wine-900">Escolha seus números</h1>
        <p className="mt-2 max-w-2xl text-wine-700">Selecione um ou mais números disponíveis. Cada número custa R$ {campaign.ticket_price.toFixed(2).replace(".", ",")}.</p>
        <div className="mt-6">
          <NumberGrid numbers={numbers} ticketPrice={campaign.ticket_price} />
        </div>
      </main>
    </PublicShell>
  );
}
